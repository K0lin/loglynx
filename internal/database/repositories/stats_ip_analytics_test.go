...
func setupTestDB(t *testing.T) (*gorm.DB, StatsRepository) {
	...
	repo := NewStatsRepository(db, &logger)
	return db, repo
}
...
func TestIPAnalyticsFiltering(t *testing.T) {
	...
	requests := []models.HTTPRequest{
		// Recent, Service A
		{RequestHash: "ip-analytics-1", ClientIP: ip, Timestamp: now.Add(-1 * time.Hour), BackendName: "svc-a", StatusCode: 200, ResponseSize: 100},
		// Recent, Service B
		{RequestHash: "ip-analytics-2", ClientIP: ip, Timestamp: now.Add(-2 * time.Hour), BackendName: "svc-b", StatusCode: 200, ResponseSize: 200},
		// Old (10 days ago), Service A
		{RequestHash: "ip-analytics-3", ClientIP: ip, Timestamp: now.Add(-240 * time.Hour), BackendName: "svc-a", StatusCode: 200, ResponseSize: 400},
	}
	assert.NoError(t, db.Create(&requests).Error)
